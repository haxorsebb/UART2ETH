...
* the ENC28J60 INT pin: the GPIO IRQ stays edge-triggered (no change to the
  ...
  `__wfi()` sets the NVIC pending bit, so `__wfi()` returns immediately; this
  race is benign. Switching the GPIO IRQ to level-triggered was considered and
  rejected for now because it requires the ISR to mask itself and the driver's
  service routine to unmask, touching a delicate part of the driver.
...
* `__sev()`/`__wfe()` instead of doorbells: no SIO resource needed, but the
  SDK lockout victim loop also uses `__wfe()`, so application events would
  spin the parked core and SDK events would wake application loops; works, but
  semantically muddier than a dedicated IRQ.
...
void wake_other_core(wake_direction_t direction) {
    multicore_doorbell_set_other_core(doorbell_for(direction));
}

static void doorbell_irq_handler(void) {
    // clear every doorbell routed to this core; nothing else to do
    multicore_doorbell_clear_current_core(doorbell_for_this_core());
}
```
