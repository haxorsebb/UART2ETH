...
|Complete copy of the system's configuration data stored as a single atomic unit in flash memory for persistence. Each Configuration Snapshot contains the entire shared_memory structure plus metadata (revision counter, SHA256 checksum, magic number) and padding to align with flash sector boundaries (4KB). The flash persistence system maintains a 4-snapshot ring buffer within the dedicated Configuration Data partition, providing wear leveling and power-loss safety through atomic write operations. This term replaces the ambiguous "page" terminology to clearly distinguish between hardware flash sectors and logical configuration storage units.
