...
=== Log Manager Synchronization Pattern

The Log Manager implements a lock-reserve-release-write pattern that ensures thread safety while minimizing lock contention between cores, with Core1 handling all output operations via periodic background tasks.

[plantuml, log-synchronization-pattern, svg]
----
@startuml
!theme plain
skinparam backgroundColor transparent

title Log Manager - Lock-Reserve-Release-Write Synchronization

participant "Core 0\nUART Logging" as C0
participant "Core 1\nNetwork Logging" as C1
participant "Shared Spinlock\nSRAM Bank" as LOCK
participant "Log Ring Buffer\nSRAM Bank" as BUFFER
participant "Core1 Background\nPrinter Task" as PRINTER
participant "USB-Serial\nStdout" as USB

== Concurrent Logging from Both Cores ==

par Core 0 Log Operation
    C0 -> C0 : Format log message:\n[12345678][0] UART0 initialized\r\n
    note left of C0 : Calculate message length\n(32 bytes including \r\n)
    
    C0 -> LOCK : Acquire spinlock\n(write_head protection)
    activate LOCK
    
    C0 -> BUFFER : Reserve space:\n- Check available space\n- Update write_head pointer\n- Reserve 32 bytes
    activate BUFFER
    
    C0 -> LOCK : Release spinlock\n(reservation complete)
    deactivate LOCK
    
    note left of C0 : Lock-free write phase\nSpace guaranteed available
    
    C0 -> BUFFER : Write message to reserved space:\n[12345678][0] UART0 initialized\r\n
    
else Core 1 Log Operation
    C1 -> C1 : Format log message:\n[12345679][1] TCP connection accepted\r\n
    note left of C1 : Calculate message length\n(35 bytes including \r\n)
    
    C1 -> LOCK : Acquire spinlock\n(wait if Core 0 holds lock)
    
    C1 -> BUFFER : Reserve space:\n- Check available space\n- Update write_head pointer\n- Reserve 35 bytes
    
    C1 -> LOCK : Release spinlock\n(reservation complete)
    
    note left of C1 : Lock-free write phase\nSpace guaranteed available
    
    C1 -> BUFFER : Write message to reserved space:\n[12345679][1] TCP connection accepted\r\n
end

== Core1 Background Output Processing ==

note left of PRINTER : Periodic 100ms timer task\nCore1 background processing

PRINTER -> PRINTER : Check for pending messages:\nread_head vs write_head

alt Messages Available
    loop Print All Pending Messages
        PRINTER -> BUFFER : Read message from read_head position
        BUFFER -> PRINTER : Return formatted message:\n[12345678][0] UART0 initialized\r\n
        
        PRINTER -> USB : Output to USB-serial stdout:\n[12345678][0] UART0 initialized\r\n
        activate USB
        
        PRINTER -> PRINTER : Advance read_head pointer\nto next message
        
        note left of PRINTER : Continue until\nread_head == write_head
    end
    
    PRINTER -> PRINTER : Update read_head position\n(all messages printed)
    
else No Messages Available
    note left of PRINTER : read_head == write_head\nNo action required
end

@enduml
----

**Fixed-Size Entry Pattern Benefits:**

1. **Predictable Memory Usage**: Fixed 16-byte entries enable precise buffer capacity calculation
2. **Eliminated Parsing Complexity**: No variable-length message boundary detection required
3. **Atomic Operations**: Single 16-byte writes are atomic on 32-bit architectures
4. **Event Loss Detection**: Per-core sequence numbering enables gap detection during debugging
5. **Version Compatibility**: Explicitly numbered event types maintain compatibility across firmware versions

**Implementation Details:**

```c
// Fixed-size log entry structure (16 bytes total)
typedef struct {
    uint32_t timestamp;             // System timestamp in milliseconds
    uint16_t event_source;          // UART0-3, NETWORK, SYSTEM, etc.
...
// Event type definitions with explicit numbering
typedef enum {
    // System events (0-99)
...
// String lookup table for formatted output
static const char* const event_format_strings[] = {
    ...
    [LOG_EVENT_UART_INIT] = "UART channel %u initialized at %u baud",
    [LOG_EVENT_TCP_CONNECT] = "TCP connection established on port %u",
    [LOG_EVENT_CONFIG_CHANGED] = "Configuration parameter %u changed",
};

// Event logging function
void log_event(uint16_t event_source, uint16_t log_level, 
               uint16_t event_type, uint32_t extra_value) {
    log_entry_t entry = {
        .timestamp = get_system_timestamp(),
        .event_source = event_source,
        .event_number = get_next_event_number(),  // Per-core counter
        .log_level = log_level,
        .event_type = event_type,
        .event_extra_value = extra_value
    };
    
    // Atomic 16-byte write to circular buffer
    write_log_entry(&entry);
}

// Core1 background formatter task (100ms periodic)
void core1_log_formatter_task(void) {
    log_entry_t entry;
    while (read_log_entry(&entry)) {
        ...
        if (format) {
            ...
            if (strstr(format, "%u")) {
                char final_msg[256];
                snprintf(final_msg, sizeof(final_msg), formatted_msg, entry.event_extra_value);
                printf("%s\n", final_msg);
            } else {
                printf("%s\n", formatted_msg);
            }
...
This scenario shows the secure firmware update process with A/B partition management and automatic rollback capability.

[plantuml, ota-update-process, svg]
----
@startuml
!theme plain
skinparam backgroundColor transparent

title OTA Firmware Update Process with A/B Partitions

participant "Management UI" as UI
participant "OTA Manager" as OTA
participant "HTTP Client" as HTTP
participant "Firmware Server" as FW_SRV
participant "Flash Manager" as FLASH
participant "Crypto Validator" as CRYPTO
participant "Global State Machine" as GSM
participant "Watchdog System" as WD
participant "ROM Bootloader" as ROM

UI -> OTA: trigger_firmware_update()
activate OTA
OTA -> GSM: request_update_mode()
activate GSM
GSM -> GSM: Set State = UPDATE_MODE
GSM -> OTA: Update Mode Approved
deactivate GSM

OTA -> HTTP: check_firmware_server()
activate HTTP
HTTP -> FW_SRV: GET /firmware/latest
activate FW_SRV
FW_SRV --> HTTP: Firmware Metadata + URL
deactivate FW_SRV
HTTP --> OTA: Update Available

OTA -> FLASH: get_inactive_partition()
activate FLASH
FLASH -> FLASH: Determine Inactive Partition
FLASH --> OTA: Partition B Available

OTA -> HTTP: download_firmware()
HTTP -> FW_SRV: GET /firmware/uart2eth-v2.0.bin
activate FW_SRV
FW_SRV --> HTTP: Firmware Binary Stream
deactivate FW_SRV

loop Download Progress
    HTTP -> FLASH: write_chunk_to_partition_b()
    FLASH -> FLASH: Write to Flash Memory
    HTTP -> UI: update_progress()
end

HTTP --> OTA: Download Complete
deactivate HTTP

OTA -> CRYPTO: verify_firmware_signature()
activate CRYPTO
CRYPTO -> CRYPTO: Extract Embedded Signature
CRYPTO -> CRYPTO: Validate Against Public Key
CRYPTO -> CRYPTO: Compute SHA-256 Hash
CRYPTO --> OTA: Signature Valid

alt Signature Valid
    OTA -> FLASH: mark_partition_b_active()
    FLASH -> FLASH: Update Partition Table
    FLASH -> FLASH: Set Boot Flags
    FLASH --> OTA: Partition Switch Configured
    
    OTA -> UI: firmware_ready_reboot_required()
    UI -> UI: Display "Reboot Required"
    
    note over UI: Manual reboot initiated by user
    UI -> GSM: system_reboot()
    GSM -> ROM: Trigger Hardware Reset
    activate ROM
    ROM -> ROM: Read Partition Table
    ROM -> ROM: Boot from Partition B
    ROM -> GSM: Start New Firmware
    activate GSM
    GSM -> WD: start_confirmation_timer()
    activate WD
    
    alt Firmware Starts Successfully
        GSM -> GSM: Complete Initialization
        GSM -> WD: disable_watchdog_confirmation()
        WD -> FLASH: mark_partition_confirmed()
        FLASH -> FLASH: Set Partition as Stable
        deactivate WD
        note over GSM: Update successful\nPartition B now primary
    else Firmware Fails to Start
        WD -> WD: Confirmation Timer Expires
        WD -> ROM: trigger_rollback()
        ROM -> FLASH: switch_to_partition_a()
        FLASH -> FLASH: Restore Previous Boot Config
        ROM -> ROM: Boot from Partition A
        note over ROM: Automatic rollback\nSystem recovers to previous version
    end
    
    deactivate ROM
    deactivate GSM
else Signature Invalid
    CRYPTO --> OTA: Signature Verification Failed
    OTA -> FLASH: erase_partition_b()
    FLASH -> FLASH: Clear Invalid Firmware
    OTA -> UI: update_failed_invalid_signature()
    note over OTA: Security preserved\nInvalid firmware rejected
end
...
note over UI, ROM: Zero-downtime updates with automatic rollback\nCryptographic security throughout process
@enduml
----


=== Management Interface Configuration Scenario

This scenario shows how system configuration changes are applied through the web-based management interface with proper validation and persistence.

[plantuml, management-configuration, svg]
----
@startuml
!theme plain
skinparam backgroundColor transparent

title Management Interface Configuration Scenario

participant "Field Technician" as TECH
participant "Web Browser" as BROWSER
participant "HTTP Server" as HTTP
participant "Authentication" as AUTH
participant "Config Validator" as VALIDATOR
participant "Configuration Manager" as CFG
participant "Core 0 UART" as C0
participant "Core 1 Network" as C1
participant "Flash Storage" as FLASH

TECH -> BROWSER: Access Device Web UI
BROWSER -> HTTP: GET http://192.168.1.100
activate HTTP
HTTP -> AUTH: check_session()
activate AUTH

alt No Valid Session
    AUTH --> HTTP: Authentication Required
    HTTP --> BROWSER: Login Page
    BROWSER -> HTTP: POST /login (credentials)
    HTTP -> AUTH: validate_credentials()
    AUTH -> AUTH: Check Username/Password
    AUTH --> HTTP: Session Token
    HTTP --> BROWSER: Set-Cookie Session
else Valid Session
    AUTH --> HTTP: Authorized
end

HTTP --> BROWSER: Main Configuration Page
deactivate AUTH

TECH -> BROWSER: Modify UART Settings
note over TECH: Change UART1: 9600→115200 baud\nChange Port mapping: 4002→4012

BROWSER -> HTTP: POST /api/uart/config
activate HTTP
HTTP -> VALIDATOR: validate_uart_config()
activate VALIDATOR
VALIDATOR -> VALIDATOR: Check Baud Rate Range
VALIDATOR -> VALIDATOR: Validate Port Assignment
VALIDATOR -> VALIDATOR: Check for Conflicts

alt Configuration Valid
    VALIDATOR --> HTTP: Validation Passed
    HTTP -> CFG: apply_uart_configuration()
    activate CFG
    CFG -> CFG: Prepare Configuration Change
    CFG -> C0: configure_uart_channel_1()
    activate C0
    C0 -> C0: Stop Current UART1 Operations
    C0 -> C0: Reconfigure Hardware Registers
    C0 -> C0: Update Baud Rate to 115200
    C0 -> C0: Restart UART1 Operations
    C0 --> CFG: UART1 Reconfigured
    deactivate C0
    
    CFG -> C1: update_port_mapping()
    activate C1
    C1 -> C1: Close Socket on Port 4002
    C1 -> C1: Open Socket on Port 4012
    C1 -> C1: Update Routing Table
    C1 --> CFG: Port Mapping Updated
    deactivate C1
    
    CFG -> FLASH: persist_configuration()
    activate FLASH
    FLASH -> FLASH: Write to Flash Memory
    FLASH -> FLASH: Verify Write Success
    FLASH --> CFG: Configuration Persisted
    deactivate FLASH
    
    CFG --> HTTP: Configuration Applied
    deactivate CFG
    HTTP --> BROWSER: Success Response
else Configuration Invalid
    VALIDATOR --> HTTP: Validation Failed
    HTTP --> BROWSER: Error Response (Conflicts)
...
TECH -> BROWSER: Verify Configuration
BROWSER -> HTTP: GET /api/system/status
HTTP -> CFG: get_current_configuration()
CFG --> HTTP: Current Settings
HTTP --> BROWSER: Status JSON
BROWSER -> BROWSER: Display Updated Configuration

note over TECH, FLASH: Configuration changes applied atomically\nPersisted for power-cycle recovery\nValidation prevents invalid states
@enduml
----