...
This scenario shows how system configuration changes are handled through the
web-based management interface. Changes are validated and persisted at once,
but take effect only at the next boot (ADR-019). The reboot is requested
explicitly by the user and executed with a grace period so the HTTP
acknowledgement is transmitted before the reset (deferred reboot, ADR-019;
reboot state flow, ADR-017).
...
note over TECH: Change Port mapping: 4002→4012

BROWSER -> HTTP: POST /config
activate HTTP
HTTP -> VALIDATOR: validate submitted values
activate VALIDATOR
VALIDATOR -> VALIDATOR: Check port range (1024-65535)

alt Configuration Valid
    VALIDATOR --> HTTP: Validation Passed
    HTTP -> CFG: store in shared memory,\nincrement revision counter
    activate CFG
    ...
    CFG --> HTTP: Saved
    deactivate CFG
    HTTP --> BROWSER: "Saved. Takes effect after reboot."
    note over C0, C1: Running UARTs, listeners and\ninterface address stay untouched\n(ADR-019: no runtime apply)
else Configuration Invalid
    VALIDATOR --> HTTP: Validation Failed
    HTTP --> BROWSER: Error Response
...
TECH -> BROWSER: Trigger Reboot
BROWSER -> HTTP: POST /reboot
activate HTTP
HTTP -> HTTP: deferred_reboot_request(\nUSER_REQUESTED)
note right of HTTP: Arms grace timer (300 ms).\nMain state stays OPERATIONAL,\nlwIP keeps being serviced.
HTTP --> BROWSER: "Rebooting." (acknowledgement\ntransmitted during grace period)
deactivate HTTP

C1 -> C1: deferred_reboot_poll()\n(grace timer expired)
C1 -> C1: MAIN_EVENT_REBOOT_REQUESTED\n→ flush config and logs,\nexecute reboot (ADR-017)

note over TECH, FLASH: Boot applies the persisted configuration:\nlisteners bound to the new ports.\nValidation prevents invalid states.
@enduml
----