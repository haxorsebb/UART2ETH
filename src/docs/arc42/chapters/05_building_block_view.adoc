...
The Log Manager provides system-wide event logging through a lock-reserve-release-write pattern that ensures thread safety while minimizing lock contention. Core1 handles all output operations via periodic background tasks to maintain consistent USB-serial output.

[plantuml, level2-log-manager-updated, svg]
----
@startuml
!theme plain
skinparam backgroundColor transparent
...
cloud "USB-Serial Interface\nStdout Output" as USB
cloud "Management Interface\nLog Query + Display" as MI

package "Log Manager - SRAM Bank Memory Layout" {
    component "Logging Interface\nBoth Cores\nLock-Reserve-Release-Write\nThread-safe message reservation" as LOG_IF #lightgreen
    
    component "Shared Memory Layout\nSRAM Bank 4\nConfig + Counters + Log Mgmt + Ring Buffer" as SHARED_MEM #lightyellow
    
    component "Ring Buffer Management\nWrite Head/Read Head\nSpinlock for Reservation\nCore1 Print Position Tracking" as RING_MGMT #lightcyan
    
    component "Log Ring Buffer\nFixed-length \r\n Messages\n[TTTTTTTT][C] Message\r\n format" as RING_BUFFER #lightyellow
    
    component "Core1 Background Printer\nPeriodic Timer Task\nPrint read_head to write_head\nUpdate read_head after printing" as PRINTER #lightblue
    
    component "Configuration Access\nShared Config Structures\nCore0 Read-only\nCore1 Read-write" as CONFIG_ACCESS #lightsteelblue
}

' External connections
C0 --> LOG_IF : Log Messages\nLock-Reserve-Write
C1 --> LOG_IF : Log Messages\nLock-Reserve-Write  
OTHER --> LOG_IF : System Events\nLower Priority
PRINTER --> USB : Formatted Output\nUSB-Serial stdout
MI --> SHARED_MEM : Config Query\nDirect Memory Access

' Internal connections
LOG_IF --> RING_MGMT : Reserve Space\nSpinlock Operations
LOG_IF --> RING_BUFFER : Write Message\nLock-free Write
RING_MGMT --> RING_BUFFER : Pointer Management\nHead/Tail Updates
PRINTER <--> RING_MGMT : Read Position\nPrint Progress Tracking
PRINTER --> RING_BUFFER : Read Messages\nSequential Access
CONFIG_ACCESS <--> SHARED_MEM : Config Data\nStructure Access
...
The lock-reserve-release-write pattern minimizes lock contention by only holding locks during pointer updates, not during actual message writing. Core1-only output ensures consistent USB-serial formatting and prevents interference between cores during logging output.
...
|*Logging Interface*
|Provides thread-safe logging API for both cores using lock-reserve-release-write pattern. Handles space reservation with spinlock protection and lock-free message writing.

|*Shared Memory Layout*
|Manages SRAM Bank 4 memory organization containing config structures, performance counters, log management variables, and ring buffer data in compile-time calculated layout.

|*Ring Buffer Management*
|Coordinates write_head/read_head pointers with spinlock protection for space reservation. Tracks Core1's print position for periodic background output processing.

|*Log Ring Buffer*
|Stores formatted log messages as fixed-length \r\n terminated strings with embedded 8-digit timestamp and core ID. Implements circular buffer with overwrite-oldest policy.

|*Core1 Background Printer*
|Periodic timer task (100ms) that outputs all pending log messages from read_head to write_head position to USB-serial stdout, then updates read_head position.

|*Configuration Access*
|Provides structured access to shared configuration data with Core0 read-only and Core1 read-write permissions for system configuration management.
|===

===== Important Interfaces

====== Logging Interface API
* **Lock-Reserve-Release-Write**: Spinlock during pointer update, lock-free during message write
* **Thread Safety**: Multiple producers (both cores), single consumer (Core1 printer)
* **Message Format**: Fixed-length format: `[TTTTTTTT][C] Message text here\r\n`
* **Overflow Policy**: Overwrite oldest messages when buffer is full

====== SRAM Bank Memory Layout
* **Config Structures**: System configuration data at bank start
* **Performance Counters**: Real-time metrics after config structures  
* **Log Management**: write_head, read_head, spinlock variables
* **Ring Buffer**: Remainder of SRAM bank filled with message buffer space
* **Compile-time Sizing**: Buffer size = Bank size - (config + counters + management)

====== Log Message Format
* **Timestamp**: 8-digit fixed-length numeric timestamp `[12345678]`
* **Core ID**: Single-digit core identifier `[0]` or `[1]`
* **Message**: Variable-length text message content
* **Terminator**: Always ends with `\r\n` for consistent parsing
* **Example**: `[00123456][1] Network interface initialized\r\n`

====== Core1 Background Processing
* **Timer Frequency**: 100ms periodic task execution
* **Print Range**: Process all messages from read_head to write_head
* **USB-Serial Output**: Single consistent output stream via stdout
* **Position Update**: Update read_head after successful printing
...
package "Configuration Manager - SRAM Bank Layout" {
    ...
    component "Flash Persistence\nLoad from flash on boot\nSave on configuration change\nA/B backup strategy" as PERSISTENCE #wheat
}
...
|*Flash Persistence*
|Manages configuration save/load operations with wear leveling, A/B backup strategy, and atomic updates to prevent configuration corruption during power failures.
|===
...
* **Revision Counter**: Incremented on each configuration change for tracking
* **SHA256 Checksum**: Calculated over entire configuration block for corruption detection
* **Load Validation**: Checksum verification during flash load with fallback to defaults
* **Save Atomicity**: Atomic update of revision + checksum + data to prevent corruption
