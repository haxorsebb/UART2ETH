...
The Global State Machine provides system-wide state coordination through a separate module that manages state variables and transitions without persisting state information. The dual-core architecture requires both shared global state and core-specific sub-states to coordinate system behavior effectively.

[plantuml, level2-state-machine-architecture, svg]
----
@startuml
!theme plain
skinparam backgroundColor transparent
...
title Level 2 - Global State Machine Architecture (Whitebox)

actor "System Startup\nInitialization sequence" as STARTUP
cloud "Core 0 Main Loop\nUART Processing" as CORE0
cloud "Core 1 Main Loop\nNetwork + Persistence" as CORE1
cloud "Error Handling\nSystem Recovery" as ERROR_SYS
cloud "Configuration Loading\nSystem Settings" as CONFIG_SYS

package "Global State Machine Module" {
    component "State Manager\nCentralized State Control\nTransition validation\nAtomic state updates" as STATE_MGR #lightgreen
    
    component "Main State Control\n16-bit Shared Variable\nINIT → CONFIGURATION → OPERATIONAL → ERROR\nSystem-wide coordination" as MAIN_STATE #lightyellow
    
    component "Core0 Sub-State Control\nUART_IDLE → UART_ACTIVE → UART_ERROR\nCore0-specific operations\nUART processing states" as CORE0_STATE #lightblue
    
    component "Core1 Sub-State Control\nNET_IDLE → NET_CONNECTED → NET_ERROR\nPERSISTENCE_ACTIVE → LOG_ACTIVE\nCore1-specific operations" as CORE1_STATE #lightcyan
    
    component "State Transition Engine\nValidation Logic\nTransition rules\nEvent-driven changes" as TRANSITION #lightpink
    
    component "State Query Interface\nRead-only State Access\nCore main loop queries\nNon-blocking operations" as QUERY_IF #lightgray
}

' External connections
STARTUP --> STATE_MGR : Initialize States\nBoot Sequence
CORE0 <--> QUERY_IF : State Queries\nControl Flow Decisions  
CORE1 <--> QUERY_IF : State Queries\nControl Flow Decisions
ERROR_SYS --> STATE_MGR : Error Events\nState Transitions
CONFIG_SYS --> STATE_MGR : Config Events\nState Transitions

' Internal connections - State Manager
STATE_MGR <--> MAIN_STATE : Global State Control\nAtomic Updates
STATE_MGR <--> CORE0_STATE : Core0 Sub-State\nControl Logic
STATE_MGR <--> CORE1_STATE : Core1 Sub-State\nControl Logic
STATE_MGR <--> TRANSITION : Transition Requests\nValidation

' Internal connections - State Query
QUERY_IF <--> MAIN_STATE : Read Main State\nVolatile Access
QUERY_IF <--> CORE0_STATE : Read Core0 Sub-State\nVolatile Access
QUERY_IF <--> CORE1_STATE : Read Core1 Sub-State\nVolatile Access

' Internal connections - Transition Engine
TRANSITION --> MAIN_STATE : State Changes\nValidated Transitions
TRANSITION --> CORE0_STATE : Sub-State Changes\nCore0 Logic
TRANSITION --> CORE1_STATE : Sub-State Changes\nCore1 Logic
...
The separate state machine module provides centralized state coordination without coupling to persistent configuration data. This architecture enables predictable system behavior through state-driven control logic while maintaining clear separation between runtime state and persistent configuration.
...
|*State Manager*
|Provides centralized control over all state variables with atomic update operations. Handles state initialization during boot sequence and coordinates system-wide state changes.

|*Main State Control*
|Manages 16-bit shared variable for system-wide state coordination. Implements INIT → CONFIGURATION → OPERATIONAL → ERROR state transitions with proper validation.

|*Core0 Sub-State Control*
|Manages Core0-specific sub-states for UART processing operations. Tracks UART_IDLE → UART_ACTIVE → UART_ERROR transitions based on UART hardware status.

|*Core1 Sub-State Control*
|Manages Core1-specific sub-states for network operations, persistence activities, and log processing. Handles NET_IDLE, NET_CONNECTED, NET_ERROR, PERSISTENCE_ACTIVE, and LOG_ACTIVE states.

|*State Transition Engine*
|Implements transition validation logic and state change rules. Prevents invalid state transitions and ensures system consistency through event-driven state changes.

|*State Query Interface*
|Provides non-blocking read-only access to current state information for core main loops. Enables state-driven control flow decisions without blocking operations.
|===
...
typedef enum {
    CORE1_NET_IDLE = 0,           // No network activity
    CORE1_NET_CONNECTED = 1,      // Active network connections
    CORE1_NET_ERROR = 2,          // Network error state
...
// State transition functions (atomic, validated)
...
// System events triggering state changes
void state_machine_on_system_error(void);
void state_machine_on_configuration_complete(void);
void state_machine_on_system_ready(void);
----

====== Core Main Loop Integration Pattern
[source,c]
----
...
* **Atomic Operations**: State transitions use atomic compare-and-swap operations
* **Memory Barriers**: Proper memory barriers ensure cache coherency between cores
