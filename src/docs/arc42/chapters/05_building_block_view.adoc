...
|Manages configuration save/load operations using 64-block flash ring buffer (8 KB per block, full 512 KB partition) for wear leveling and power-loss safety. Implements deferred write-back with SHA256 integrity verification and sector-level updates. Uses partition ID=3 discovered via bootrom APIs.
|===
...
* **64-Block Ring Buffer**: Circular buffer of 8 KB flash blocks (2 sectors each) spanning the whole configuration partition for wear leveling
...
* **Startup Recovery**: Load all 64 blocks, select highest revision + valid checksum
* **Deferred Writes**: Core1 write-back limited to maximum once per 120 seconds
...
|*Write Controller*
|Manages deferred write-back strategy with Core1 integration. Limits write frequency to maximum once per 120 seconds and only writes when shared memory revision counter changes.

|*Flash Ring Buffer Manager*
|Implements 64-block circular buffer (8 KB per block, 512 KB total) for wear leveling. Tracks current write position, manages block allocation sequence, and maintains ring buffer state at runtime.
...
|*Recovery System*
|Handles startup configuration loading by validating all 64 ring buffer blocks. Selects block with highest revision and valid checksum, implements factory reset fallback via `shared_memory_force_reinit()` for total corruption.
...
* **Block Size**: 8 KB (2 × 4096-byte flash sectors); 64 blocks fill the 512 KB partition
* **Data Section**: Complete `shared_memory_layout_t` structure (raw binary), log ring capacity derived from `TOTAL_SHARED_MEM_USABLE_SIZE`
...
* **Frequency Limit**: Maximum once per 120 seconds via Core1 timer
...
* **Ring Advancement**: Automatic progression to next block in 64-block ring
* **Safe Flash Access**: Uses `flash_safe_execute()` for interrupt-safe operations

====== Recovery Interface
* **Block Scanning**: Read all 64 ring buffer blocks during startup
...
[source,c]
----
// Flash persistence block (8 KB = FLASH_PERSISTENCE_BLOCK_SIZE, 2 × 4096-byte sectors)
// Note: sha256_checksum is the first field of shared_memory_layout_t to simplify
// exclusion during hash calculation. The hash covers all fields after it.
typedef union {
    struct {
        // Complete shared memory structure (raw binary):
        // checksum, magic, revision, config, counters, log ring
        shared_memory_layout_t shared_memory_data;
    };
    // Pads the block to 8 KB; the log ring grows into this space
...
typedef struct {
    uint8_t  current_write_block;         // 0-63, next block to write
    uint8_t  last_valid_block;            // 0-63, last successfully read block
...
1. **Corruption Detection**: If all 64 ring buffer blocks fail SHA256 verification, or user-requested
2. **Factory Reset Execution**: Erase all 64 blocks, call `shared_memory_force_reinit()` to restore defaults
...
2. **Block Validation**: Read all 64 blocks, verify magic numbers and SHA256 checksums
...
- Test ring buffer wraparound with real 8 KB block operations
...
- Validate 120-second deferred write-back timing with Core1 main loop

**Test Coverage Requirements:**
- Sector-by-sector flash operations (read/write/erase)
- Ring buffer advancement and wraparound across 64 blocks
